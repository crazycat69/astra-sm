...
#ifndef _WIN32
#   include <arpa/inet.h> /* need this for htonl() */
#endif
 ...
 */
#define SHA_SRC(t) asc_get_be32((uint8_t *)block + (t)*4)
#define SHA_MIX(t) SHA_ROL(W((t)+13) ^ W((t)+8) ^ W((t)+2) ^ W(t), 1)

#define SHA_ROUND(t, input, fn, constant, A, B, C, D, E) \
    do { \
        const unsigned int __temp = input(t); \
        setW(t, __temp); \
        E += __temp + SHA_ROL(A, 5) + (fn) + (constant); \
        B = SHA_ROR(B, 2); \
    } while (0)
...
static
void sha1_block(sha1_ctx_t *ctx, const void *block)
{
...
void au_sha1_final(sha1_ctx_t *ctx, uint8_t digest[SHA1_DIGEST_SIZE])
{
    ...
    i = ctx->size & 63;
    au_sha1_update(ctx, pad, 1 + (63 & (55 - i)));
    au_sha1_update(ctx, padlen, 8);

    /* Output hash */
    for (i = 0; i < 5; i++)
        asc_put_be32(digest + i * 4, ctx->H[i]);
}
